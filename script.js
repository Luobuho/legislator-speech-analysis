...
function calculateTopicSimilarityGroups() {
    ...
    // 建立立委-主題向量矩陣
    const legislatorVectors = [];
    const legislatorNames = [];

    // 獲取所有主題ID
    const allTopicIds = [...new Set(currentData.legislators.flatMap(leg =>
    ...
    currentData.legislators.forEach(legislator => {
        ...
        legislatorVectors.push(vector);
        legislatorNames.push(legislator.name);
    });

    // 實現K-means聚類
    const clusters = performKMeansClustering(legislatorVectors, legislatorNames, numGroups);

    return clusters;
}

// K-means聚類實現
function performKMeansClustering(vectors, names, k) {
    ...
    for (let i = 0; i < assignments.length; i++) {
        ...
        const legislator = currentData.legislators.find(leg => leg.name === names[i]);
        if (legislator) {
            groups[clusterId].push(legislator);
        }
    }
